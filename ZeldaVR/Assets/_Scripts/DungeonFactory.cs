...
public class DungeonFactory : Singleton<DungeonFactory>
{
    ...
    public void AssignWallMaterials(DungeonRoom dr)
    {
        ...
        dr.wall_N.renderer.material = GetWallMaterial(DungeonRoomInfo.WallDirection.North, info.northWallType);
        dr.wall_E.renderer.material = GetWallMaterial(DungeonRoomInfo.WallDirection.East, info.eastWallType);
        dr.wall_S.renderer.material = GetWallMaterial(DungeonRoomInfo.WallDirection.South, info.southWallType);
        dr.wall_W.renderer.material = GetWallMaterial(DungeonRoomInfo.WallDirection.West, info.westWallType);

        dr.wall_N2.renderer.material = GetWallMaterial(DungeonRoomInfo.WallDirection.North, DungeonRoomInfo.WallType.Top);
        dr.wall_E2.renderer.material = GetWallMaterial(DungeonRoomInfo.WallDirection.East, DungeonRoomInfo.WallType.Top);
        dr.wall_S2.renderer.material = GetWallMaterial(DungeonRoomInfo.WallDirection.South, DungeonRoomInfo.WallType.Top);
        dr.wall_W2.renderer.material = GetWallMaterial(DungeonRoomInfo.WallDirection.West, DungeonRoomInfo.WallType.Top);
        ...
        dr.floor.renderer.material = info.floorMaterial;
        dr.ceiling.renderer.material = defaultFloor;
    }
    ...
    void ProcessBlocks()
    {
        foreach (Transform block in blocksContainer)
        {
            ...
            if (block.gameObject.layer != LayerMask.NameToLayer("InvisibleBlocks"))
            {
                ...
                block.renderer.material.mainTextureScale = new Vector2(1, shortBlockHeight);
            }
        }
    }
    ...
    void CreateHolesInFloor()
    {
        ...
        foreach (Transform hole in holeMarkersContainer)
        {
            ...
            {
                origTexture = dr.floor.renderer.material.mainTexture as Texture2D;
            }
            ...
            {
                dr.Info.PushBlockChangeFloorMaterial = new Material(dr.floor.renderer.material);
                dr.Info.PushBlockChangeFloorMaterial.mainTexture = newTexture;
            }
            else
            {
                dr.Info.floorMaterial.mainTexture = newTexture;
                dr.floor.renderer.material.mainTexture = newTexture;
            }
        }
    }
