...
public class OVRPlayerController : OVRComponent
{
    ...
    public OVRCameraController CameraController_OVR = null;
    public GFLCamera_Zelda CameraController_GFL = null;
    ...
    public Vector3 ForwardDirection { get { return DirXform.forward; } }
    ...
    {      
        get 
        {
            return (ZeldaConfig.Instance.isGflBuild ? CameraController_GFL.LineOfSight : CameraController_OVR.LineOfSight); 
        }
    }
    ...
    void InitCameras()
    {
        bool useGFLCamera = ZeldaConfig.Instance.isGflBuild;
        if (CameraController_GFL != null)
        {
            CameraController_GFL.gameObject.SetActive(useGFLCamera);
        }
        if (CameraController_OVR != null)
        {
            CameraController_OVR.gameObject.SetActive(!useGFLCamera);
        }
    }

    void AttachWeaponContainers()
    {
        Transform camTrans = ZeldaConfig.Instance.isGflBuild ? CameraController_GFL.transform : CameraController_OVR.transform;
        weaponContainerLeft.parent = camTrans;
        weaponContainerRight.parent = camTrans;
    }
    ...
    public virtual void UpdateMovement()
    {
        ...
        if (DirXform != null)
        {
            float leftAxisY = OVRGamepadController.GPC_GetAxis((int)OVRGamepadController.Axis.LeftYAxis);

            float leftAxisX = OVRGamepadController.GPC_GetAxis((int)OVRGamepadController.Axis.LeftXAxis);
        ...
        float rightAxisX = OVRGamepadController.GPC_GetAxis((int)OVRGamepadController.Axis.RightXAxis);
    ...
    public virtual void UpdatePlayerForwardDirTransform()
    {
        if ((DirXform != null))
        {
            Quaternion camRot = ZeldaConfig.Instance.isGflBuild ? CameraController_GFL.transform.rotation : CameraController_OVR.transform.rotation;

            Quaternion q = Quaternion.identity;
            q = Quaternion.Euler(0.0f, YfromSensor2, 0.0f);
            DirXform.rotation = q * camRot;
        }
    }
    ...
    public void SetCameras()
    {
        if (!ZeldaConfig.Instance.isGflBuild)
        {
            if (CameraController_OVR != null)
            {
                ...
                CameraController_OVR.SetOrientationOffset(OrientationOffset);
                CameraController_OVR.SetYRotation(YRotation);
            }
        }
    }
